...
func TestPostgresStore(t *testing.T) {
	...
	defer raw.Close()
	if _, err := raw.Exec(`TRUNCATE runs, journal, signals, cancellations`); err != nil {
		t.Fatal(err)
	}
...
func TestPostgresCanceller(t *testing.T) {
	d := dsn(t)
	...
	defer raw.Close()
	if _, err := raw.Exec(`TRUNCATE cancellations`); err != nil {
		t.Fatal(err)
	}
